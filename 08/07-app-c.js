...
function verifyToken(token, secretKey) {
    try {
        ...
        if (((Date.now()/1000) - decodeToken.iat) < COOKIE_AGE*60) {
            return true;
        }
        else {
            return false;
...
function parseCookie(cookie, key){
    let splited_cookie = cookie.split(';');
    let splited_cookie_param = splited_cookie[0].split('=');
    if (splited_cookie_param[0] === key)
    {
        return splited_cookie_param[1];
    }
}
    ...
    readFile('./users.json', function (error, data) {
        if (error) {
            ...
            if (!userFound) {
                ...
                let token = jwt.sign(request.data, SECRET_KEY)
                console.log('Token: ', token);
                write(response, 200, 'Token: ' + token.toString(), 'token='+token+'; Max-Age='+COOKIE_AGE*60);
            }
        }
    })
})
