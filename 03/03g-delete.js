    ...
    readFile('./data.json', function(error, data){
        if (error){
            ...
            for (let i = 0; i < objData.data.length; i++){
                if (objData.data[i].name === input[0]){
                    found = true;
                    delete objData.data[i];
                }
            }
            writeFile('./data.json', JSON.stringify(objData), function(error, data){
                if (error){
                    console.log('Error:', error);
                }
                else{
                    console.log('Successsfuly Deleted Item...');
                }
            })
            if(!found){
                console.log('Item Not Found...');
